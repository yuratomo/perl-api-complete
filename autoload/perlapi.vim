...
let g:perl_complete_item_len = 30
let s:complete_mode = s:MODE_CLASS
let s:type = ''
let s:parts = []
let s:last_list = []

let g:perl_access_modifier = [
  \ 'public',
  \ 'private',
  \ 'protected',
  \ 'internal',
...
function! perlapi#complete(findstart, base)
  if a:findstart
    let line = line('.')
    let cur = col('.') - 1
    ...
    elseif s:complete_mode == s:MODE_STATIC
      ...
      if len(s:parts) >= 1 
        ...
        for ns in perlapi#getNamespaces()
          if ns =~ '^' . variable
        ...
        if match_ns == 1 || s:parts[0] == 'perl'
          let s:type = substitute(type, '.*\.', '', '')
        elseif perlapi#isEnumExist(s:parts[0])
...
function! s:enum_member_completion(tag, base, res)
  if !perlapi#isEnumExist(a:tag)
    return
  endif

  let item = perlapi#getEnum(a:tag)
  for member in item.members
   if member.name =~ '^' . a:base
      call add(a:res, perlapi#member_to_compitem(item.name, member))
    endif
  endfor
endfunction
...
function! s:class_member_completion(base, res, type)
  ...
  for part in parts
    ...
    if !perlapi#isClassExist(class)
      if !perlapi#isEnumExist(class)
        let item = perlapi#getTag(class)
        if empty(item)
          if exists('item')
            unlet item
          endif
        ...
        let item = perlapi#getEnum(class)
      endif
    else
      let item = perlapi#getClass(class)
    endif
    ...
    while 1
      ...
      if has_key(item, 'extend')
        if perlapi#isClassExist(item.extend)
          let item = perlapi#getClass(item.extend)
        else
          let item = perlapi#getTag(item.extend)
          if empty(item)
            unlet item
            break
          endif
  ...
  if exists('item')
    ...
    elseif a:type == 1
      ...
      if !has_key(s:primitive_dict, item.name)
        let newitem = perlapi#member_to_compitem('new ' . item.name . '(', {})
        let newitem.menu = 'create new instance'
        call add(a:res, newitem)
        let newitem = perlapi#member_to_compitem('(' . item.name . ')', {})
...
function! s:find_type(start_line, var)
  ...
  if perlapi#isClassExist(result.class)
...
function! s:attr_completion(tag, base, res, static)
  if !perlapi#isClassExist(a:tag)
    return
  endif

  let item = perlapi#getClass(a:tag)
  for member in item.members
...
function! s:abbr(str)
  if len(a:str) > g:perl_complete_item_len
    return a:str[0 : g:perl_complete_item_len] . '...'
  endif
  return a:str
endfunction

function! perlapi#member_to_compitem(class, member)
  if empty(a:member)
    return {
      \ 'word' : a:class,
      \ 'abbr' : s:abbr(a:class),
...
let s:class = {}
function! perlapi#class(name, extend, members)
  let s:class[ a:name ] = {
    \ 'name'   : a:name,
    \ 'kind'   : 't',
    \ 'extend' : a:extend,
  ...
  if exists('s:parent') && index(s:parent.members, a:name) == -1
    call add(s:parent.members, perlapi#field(0, 1, a:name, a:name))
  endif
endfunction
function! perlapi#interface(name, extend, members)
  call perlapi#class(a:name, a:extend, a:members)
endfunction

let s:function = []
function! perlapi#function(name, signature, retval)
  call add(s:function, 
    \ {
...
function! perlapi#namespace(name, detail)
  return {
    \ 'type'   : s:TYPE_METHOD,
    \ 'kind'   : 'f', 
    \ 'name'   : a:name,
...
function! perlapi#namespace(ns)
  ...
  for part in parts
    ...
    "  call add(s:parent.members, perlapi#field(part, ''))
    "endif

    if !perlapi#isClassExist(part)
      call s:namespace_item(part, '', [])
    endif

    " last namespace (for perlapi#class)
    let s:parent = perlapi#getClass(part)
  endfor
endfunction

let s:keyword = []
function! perlapi#keyword(name, detail)
  call add(s:keyword, 
    \ {
    \ 'type'      : s:TYPE_KEYWORD,
    \ 'name'      : a:name,
...
function! perlapi#method(name, detail, class)
  return {
    \ 'type'   : s:TYPE_METHOD,
    \ 'kind'   : 'f', 
    \ 'name'   : a:name,
...
function! perlapi#field(static, public, name, class)
  return {
    \ 'type'   : s:TYPE_FIELD,
    \ 'kind'   : 'v', 
    \ 'name'   : a:name,
...
let s:enum = {}
function! perlapi#enum(name, members)
  let s:enum[ a:name ] = {
    \ 'type'   : s:TYPE_ENUM,
    \ 'name'   : a:name,
    \ 'kind'   : 't',
...
function! perlapi#isMethod(member)
...
function! perlapi#isClassExist(name)
  return has_key(s:class, a:name)
endfunction

function! perlapi#getClass(name)
  return get(s:class, a:name)
endfunction

function! perlapi#getSuperClassList(name, list)
  if !perlapi#isClassExist(a:name)
    return
  endif

  let item = perlapi#getClass(a:name)
  if item.extend != '' && item.extend != '-'
    call add(a:list, item.extend)
    call perlapi#getSuperClassList(item.extend, a:list)
  endif
endfunction

function! perlapi#getTag(name)
  ...
  for tag in tags
    ...
    if index(g:perl_access_modifier, ttype) >= 0
      let ttype = name
    endif
    call add(class.members, perlapi#field(0, 1, name, ttype))
  endfor
  return class
endfunction

function! perlapi#isEnumExist(name)
  return has_key(s:enum, a:name)
endfunction

function! perlapi#getEnum(name)
  return get(s:enum, a:name)
endfunction

function! perlapi#getNamespaces()
  return s:namespace
endfunction
...
function! perlapi#showRef()
  if !exists('g:perlapi#statusline')
    let g:perlapi#statusline = &statusline
  endif
  ...
  \ 'line'  : line('.'),
  \ }
  call perlapi#nextRef()

  augroup perlapi
    au!
    au InsertLeave  <buffer> call perlapi#clearRef()
    au CursorMovedI <buffer> call perlapi#checkLineForRef()
  augroup END
  return ""
endfunction
...
function! perlapi#nextRef()
  return s:prevNextRef(1)
endfunction
function! perlapi#prevRef()
  return s:prevNextRef(-1)
endfunction
...
function! perlapi#checkLineForRef()
  if exists("b:ref")
    if b:ref.line != line('.')
      call perlapi#clearRef()
    endif
  endif
endfunction
...
command! -nargs=1 -complete=customlist,perlapi#load_list PerlApiLoad :call perlapi#load(<f-args>)
function! perlapi#load_list(A, L, P)
  let items = []
  for item in g:perlapi#delay_dirs
    if item =~ '^'.a:A
      call add(items, item)
    endif
  endfor
...
function! perlapi#load(sub)
  let rtp = filter(split(&runtimepath, ','), 'v:val =~ a:sub')
  let files = split(globpath(join(rtp, ','), 'autoload/perlapi/*.vim'), '\n')
  for file in files
    if file
      continue
    endif
    exe 'echo "[perl-complete] load ' . substitute(file, '^.*\','','') . '"'
    redraw
    exe 'so ' . file
  endfor
  echo '[perl-complete] loaded!'
  call remove(g:perlapi#delay_dirs, a:sub)
endfunction
...
if !exists('s:dictionary_loaded')
  ...
  " delay directories
  if !exists('g:perlapi#delay_dirs')
    let g:perlapi#delay_dirs = []
  end
  let rtp = split(&runtimepath, ',')
  for dir in g:perlapi#delay_dirs
